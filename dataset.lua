...
dataset.train_dataset_bin = "/data/hossamkasem/13-CS/1-Dataset/1-Transformed/3-Natural/1-R/1-0.5/combined_train_half_initial.bin"
dataset.validation_dataset_bin = "/data/hossamkasem/13-CS/1-Dataset/1-Transformed/3-Natural/1-R/1-0.5/combined_validation_half_initial.bin"
dataset.test_dataset_bin = "/data/hossamkasem/13-CS/1-Dataset/1-Transformed/3-Natural/1-R/1-0.5/combined_test_half_initial.bin"
...
--  dataset.download_generate_bin()
 ...
 -- local train_label = torch.load(dataset.train_label_bin)
...
function dataset.normalize_local(dataset)
  ...
  local dataset_label_size = dataset.label:size(1)
  ...
  for i=1,dataset_size,batch do
    local local_batch = math.min(dataset_label_size,i+batch) - i
    local normalized_images = norm:forward(dataset.label:narrow(1,i,local_batch))
    dataset.label:narrow(1,i,local_batch):copy(normalized_images)
  end
end
...
prune_dataset = function(dataset, nbr_examples)
  ...
  local subset_label = torch.Tensor(nbr_examples,3,48,48)
